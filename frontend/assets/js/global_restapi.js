...
function sendSMSToServer(phoneNumber) {
    ...
    $.ajax({
        type: 'POST',
        url: MY_API_URL + 'sms/api_demo/SendTemplateSMS.php', //rest API url
        dataType: 'json',
...
function sendRegisterRequest(phoneNumber, passwd, servantPhone) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {'phone': phoneNumber, 'password': passwd, 'invited': servantPhone}, // set function name and parameters
        success: function (data) {
            if (HLC_LOGIN_MODE == HLC_SIMUL_MODE) {
                ...
                switch (parseInt(data.err_code)) {
                    ...
                    case 2:
                        ...
                        setRegisterStatus(true);
                        if(servantPhone!='') setCouponStatus(servantPhone);
                        else setCouponStatus('');
                        setTimeout(function () {
                            sendLoginRequest(phoneNumber, '',1);
                        },1000);
                        return;
                        break;
                    default:
                ...
                // showAuthRequire('注册成功,<br>请您及时进行认证！', '立即认证', '跳过')
                if(data.err_message!='') showNotifyAlert(data.err_message);
                showNotifyAlert('注册成功');
                if(servantPhone!='') setCouponStatus(servantPhone);
                else setCouponStatus('');
                setTimeout(function () {
                    sendLoginRequest(phoneNumber, '',1);
    ...
    $.ajax({
            ...
            success: function (data) {
                if (HLC_LOGIN_MODE == HLC_SIMUL_MODE) {
                    ...
                    setUserId(myUserInfo.user_id)
                    setSessionMyInfo(myUserInfo)
                    location.href = 'home.php';
                } else if (data.status == false) { // if request is failed,
                    ...
                    switch (parseInt(data.user_data.user_status)) {
                        case 1:
                    ...
                    if (status == 1) { // normal login request sent
                        setSessionPassword(passwd);
                        setRegisterStatus(true);
                        localStorage.setItem('hlc_token', data.user_data.token);
                        setPhoneNumber(phoneNumber);
                        setUserId(data.user_data.user_id);
                        var delay_time = 10;
                        if (localStorage.getItem('isCouponShowed')=='0' && data.user_data.coupon == 1) {
                            showNotifyAlert('欢迎使用互助贝,<br>赠送0.1mBTC代金券, 满300元使用');
                            localStorage.removeItem('isCouponShowed');
                            delay_time = 3000;
                        }
                        setMySessionWallet(data.user_data.wallet);
                        ...
                        sessionStorage.clear({});
                        localStorage.setItem('isLogout', '1');
                        if (coup != 0) setMySessionWallet(coup);
                        if (localUserInfo != undefined) localStorage.setItem('user_register_detail', localUserInfo);
                        if (my_hlc_token != undefined) localStorage.setItem('hlc_token', my_hlc_token);
                        if (shipper_data != undefined) localStorage.setItem('shipper_data', shipper_data);
                        if (shipper_user_number != undefined) localStorage.setItem('shipper_user_number', shipper_user_number);
                    ...
                    } else { // get userInfo from page using.(status !=1 and !=2 )
                        if (localStorage.getItem('hlc_token') != data.user_data.token) { // other man logged in
                            login_token_counter++;
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == false) {
                ...
                if (sendTxt != '') {
                    showNotifyAlert('您的退款申请已提交,<br>我们将会尽快处理！');
                    setTimeout(function () {
                        history.go(-1);
                        clearTimeout();
